...
public class EmpDAO implements EmpDAO_interface{
	...
	public List<EmpVO> findEmpVOByVO(QueryVO vo) {
		...
		SqlSession session = MyBaitsUtil.getSessionFactory().openSession();
		//2.用SqlSession查詢列表  用selectList
		List<EmpVO> emps = session.selectList("com.emp.model.EmpDAO_interface.findEmpVOByVO",vo);//EmpDao.xml的namespace(KEY)
		//3.釋放資源
		session.close();
		return emps;
	}
	...
	public List<EmpVO> findEmpVOByCondition(EmpVO empVO) {
		...
		SqlSession session = MyBaitsUtil.getSessionFactory().openSession();
		//2.用SqlSession查詢列表  用selectList
		List<EmpVO> emps = session.selectList("com.emp.model.EmpDAO_interface.findEmpVOByCondition",empVO);//EmpDao.xml的namespace(KEY)
		//3.釋放資源
		session.close();
		return emps;
	}
	...
	public List<EmpVO> findEmpNos(QueryVO vo) {
		...
		SqlSession session = MyBaitsUtil.getSessionFactory().openSession();
		//2.用SqlSession查詢列表  用selectList
		List<EmpVO> emps = session.selectList("com.emp.model.EmpDAO_interface.findEmpNos",vo);//EmpDao.xml的namespace(KEY)
		//3.釋放資源
		session.close();
		return emps;
	}
