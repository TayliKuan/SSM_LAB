...
<%
	...
	List<LessonVO> lessonlist = lessSvc.getCoachLesson(coano);
		...
		<div class="table-responsive-sm table-hover table-warning">
			<table class="table align-items-center">
				...
				<c:forEach var="lessonVO" items="${lessonlist}">
					<tr>
						<td class="align-middle">${lessonVO.lessno}</td>
						<td class="align-middle">${lessonVO.lessprice}</td>
						<td class="align-middle">${lessonVO.lesssta}</td>
						...
						<form action="<%=request.getContextPath()%>/redemption/redemption.do" method="post" class="form1">
							...
							<c:if test="${lessonVO.lesssta =='未成團'|| lessonVO.lesssta =='下架'|| lessonVO.lesssta =='已結束'}"> disabled="disabled"
                   			</c:if>>
