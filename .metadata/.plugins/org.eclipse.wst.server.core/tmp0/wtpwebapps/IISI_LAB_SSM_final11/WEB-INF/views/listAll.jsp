...
<c:if test="${not empty OK}">
			<script>
				swal("產出PDF成功","請至D槽 尋找printAll.pdf");
 			</script> 
		</c:if>
<!-- -----------------------------------------------header 開始------------------------------------------------ -->
	 <div class='wrapper'>
...
<!-- ------------------------------------------------複合查詢 開始------------------------------------------------ -->
    <div class="container">
	<form:form action="user_findUserByCondition.action" method="post" modelAttribute="Condition">
            ...
            <div class="btn-toolbar mb-3 justify-content-end" role="toolbar" aria-label="Toolbar with button groups">
                <div class="btn-group mr-2" role="group" aria-label="First group">
                  ...
                  <form:form action="user_deleteMany.action" method="post" id="formdel">
                   ...
                   <form:form action="user_pdf.action" method="post">
                  <button type="submit" class="btn btn-secondary">產出Pdf</button>
                  </form:form>
                  <button type="button" class="btn btn-secondary">產出Excel</button>
                </div>
            </div>
            
             <div class="btn-toolbar mb-3 justify-content-start">
			  <button type="button" class="btn btn-secondary" id="selectAll">全選</button>
			   <button type="button" class="btn btn-secondary" id="unSelect">全不選</button>
			 </div>
 <!-- ---------------------------------------------新增 批次刪除 結束------------------------------------------------ -->
  <!-- ---------------------------------------------迴圈全部 開始------------------------------------------------ -->
...
$(document).ready( function () {
		...
		$(".deleteMany:checkbox").attr("checked", true);
    });
    $("#unSelect").click(function(){
		$(".deleteMany:checkbox").attr("checked", false);
		});
	});
</script>
<!-- ------------------------------------------動態生成 批次刪除 結束------------------------------------------------ -->
