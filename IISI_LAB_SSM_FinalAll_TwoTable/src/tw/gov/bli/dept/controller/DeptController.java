...
//------------------------------------------------------------------------------------------------------------------//
	// enter修改單位
	@RequestMapping("updateOneDept_enter.action")
	public String updateOneDept_enter( Dept dept,Model model,@RequestParam String deptno) {
		dept = deptService.findByPrimaryKey(deptno);
		model.addAttribute("dept",dept);
		model.addAttribute(deptno);
		return "updateOnlyDept";
	}
	
	// update修改單位
	@RequestMapping("updateOneDept.action")
	public String updateOneDept(@Valid Dept dept,BindingResult result,Model model,@RequestParam String deptno) {
		
		// 驗證錯誤
		if (result.hasErrors()) {
			model.addAttribute("message", "有錯誤!!! 請再次核對");
			return "updateOnlyDept";//要修改的頁面
		}
...
//------------------------------------------------------------------------------------------------------------------//
	//enter 新增USER
		...
		public String addUser(Model model,@RequestParam String deptno,@RequestParam String action) {			
			model.addAttribute("user",new User());
			model.addAttribute("deptno",deptno);
			logger.debug("有進來");
			logger.debug(deptno);
			return "insertUserInDept";//要新增的頁面
		}	
	// 新增USER
		@RequestMapping("saveUser.action")
		public String aveUser(@Valid User user,BindingResult result,Model model,@RequestParam String deptno,@RequestParam String action) {
			if (result.hasErrors()) {
				model.addAttribute("message", "有錯誤!!! 請再次核對");
				return "insertUserInDept";//要新增的頁面
			}
...
//------------------------------------------------------------------------------------------------------------------//
	//enter修改User
	@RequestMapping("updateOneUser_enter.action")
	public String updateOneUser_enter(User user,Model model,@RequestParam String uno) {
		user = userService.findByPrimaryKey(uno);
		model.addAttribute("user",user);
		model.addAttribute(uno);
		return "updateOnlyUser";
	}
	
	//update修改User
		@RequestMapping("updateOneUser.action")
		public String updateOneUser(@Valid User user,BindingResult result,Model model,@RequestParam String uno) {
			if (result.hasErrors()) {
				model.addAttribute("message", "有錯誤!!! 請再次核對");
				return "updateOnlyUser";//要修改的頁面
			}
			logger.debug(uno);
			user.setUno(uno);
...
//------------------------------------------------------------------------------------------------------------------//
	...
	public String delAll(Model model,@RequestParam String deptno) {
		//database change  to "ON DELETE CASCADE"
